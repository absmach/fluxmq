...
## Cluster compose used by scripts and tests

Compose file: `docker/docker-compose-cluster.yaml`
...
- Uses `ghcr.io/absmach/fluxmq:latest`.
- No local image build is performed by default.
- Named volumes are used for per-node data; use reset/clean targets to remove them.
...
Cluster lifecycle (general aliases):

- `make run-cluster`
- `make run-cluster-down`
- `make run-cluster-clean`
- `make run-cluster-ps`
- `make run-cluster-logs [SERVICE=node1|node2|node3]`
...
- `PERF_COMPOSE_FILE`: override compose path (default: `docker/docker-compose-cluster.yaml`)
