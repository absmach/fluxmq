...
The following is a set of guidelines to contribute to FluxMQ and its libraries, which are
...
Before raising a new issue, check [our issue list](https://github.com/absmach/fluxmq/issues) to determine if it already contains the
...
```
git clone https://github.com/<your-username>/fluxmq.git
cd fluxmq
git remote add upstream https://github.com/absmach/fluxmq.git
```
...
Create a new topic branch from `main` using the naming convention `fmq-[issue-number]`
to help us keep track of your contribution scope:

```
git checkout -b fmq-[issue-number]
```
...
```
git push origin fmq-[issue-number]
```
