...
func TestSessionStore(t *testing.T) {
	...
	if got.ClientID != session.ClientID {
		t.Errorf("ClientID mismatch")
	}
	if got.Version != session.Version {
		t.Errorf("Version mismatch")
	}
	if got.ReceiveMaximum != session.ReceiveMaximum {
		t.Errorf("ReceiveMaximum mismatch")
	}

	// Test GetExpired
	session.Connected = false
	session.DisconnectedAt = time.Now().Add(-2 * time.Hour)
	session.ExpiryInterval = 1800 // 30 minutes
	s.Save(session)

	expired, err := s.GetExpired(time.Now())
	if err != nil {
		t.Fatalf("GetExpired failed: %v", err)
	}
	if len(expired) != 1 || expired[0] != "client1" {
		t.Errorf("Expected client1 to be expired, got %v", expired)
	}

	// Test Lock
	unlock, err := s.Lock("client1")
	if err != nil {
	...
	// Test List
	s.Save(&store.Session{ClientID: "c1"})
	s.Save(&store.Session{ClientID: "c2"})
	list, err := s.List()
	if err != nil {
		t.Fatalf("List failed: %v", err)
	}
