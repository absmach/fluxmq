...
cluster:
  enabled: true
  allow_insecure: true # Development-only plaintext opt-in; use TLS in production.
  node_id: "broker-1"
...
Raft affects durable queues only.

> **Not available in this release.** `cluster.raft.enabled: true` is refused at
> startup in FluxMQ 1.0. The keys parse so a configuration prepared for a later
> release still loads; the rest of this section describes the reserved shape.

If `cluster.raft.enabled` is true, FluxMQ starts one or more Raft replication groups. Queues can be assigned to a specific group (sharding), otherwise they use the default group.
...
`forward` is the default and works with or without queue Raft. `replicate` requires
`cluster.raft.enabled: true`; startup fails instead of silently falling back when
...
For the conceptual model and tradeoffs, see [Queue Replication Groups (Raft)](/architecture/raft-groups).
