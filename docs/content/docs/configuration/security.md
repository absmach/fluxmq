...
and the identity it returns must exactly equal the verified leaf certificate's
subject common name (CN). The password is never stored in or compared with the
certificate.
...
There is no prefix removal or configurable template. If authentication returns
