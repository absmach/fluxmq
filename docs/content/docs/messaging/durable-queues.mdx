...
For an overview of all three queue types (including ephemeral queues), see [Queue Types](/concepts/queues).
...
To *address* a queue, yes. To have a message stored in one, not necessarily.

- Publish to `$queue/orders/...` goes through the queue manager: log storage, consumer groups, and acks. The `$queue/` prefix is what activates queue semantics, whether the target is an ephemeral, durable, or stream queue.
- Publish to `sensors/temp` goes through normal pub/sub routing — subscriptions, retained, cluster pub/sub — **and** is additionally appended to any queue whose configured `topics` pattern matches it.

So a queue bound to `sensors/#` captures ordinary publishes to `sensors/temp` without the publisher addressing it or knowing it exists. Only a `$queue/` publish is *routed* to the queue manager; a matching pattern is what gets an ordinary publish *stored*. See [Capturing Ordinary Topics](#capturing-ordinary-topics).
...
- Patterns are matched against **every** publish, not only `$queue/...` addresses. See [Capturing Ordinary Topics](#capturing-ordinary-topics).
- If **no queue matches**, FluxMQ creates an **[ephemeral queue](/concepts/queues#ephemeral-queue)** whose name and pattern equal the topic, then appends. Ephemeral queues are in-memory, best-effort, and cleaned up after the last consumer disconnects.
...
In production, prefer explicit `queues:` configuration so queue names, types, and bindings are stable. Ephemeral queues are a development convenience — they prevent silent message loss but don't provide durability or delivery guarantees. See [Queue Types](/concepts/queues) for when to use each type.
...
## Capturing Ordinary Topics

A queue's `topics` patterns are not restricted to `$queue/...`. A pattern may
bind an ordinary pub/sub topic, and every publish matching it is appended to the
queue — on MQTT, AMQP 0.9.1 and AMQP 1.0 alike, whatever the publisher intended.

```yaml
queues:
  - name: "messages"
    topics:
      - "m/#"          # captures ordinary pub/sub traffic, not just $queue/messages/...
```

With that queue configured, an MQTT client publishing `m/acme/c/temp` reaches its
normal subscribers **and** has the message persisted in `messages`. Capture is in
...
- **The queue's `topics` pattern is what grants persistence.** No publisher-side
  permission distinguishes "publish this topic" from "persist this topic" — see
  [Local principals and capture](/configuration/security#local-principals-and-capture).
- **Only the ingress node captures.** A publish forwarded to another node for
  fan-out arrives through the cluster's forward path, which delivers to local
  subscribers without re-appending, so a message is stored once per matching queue.
- **Explicit queue publishes are not captured twice.** A publish addressed to
  `$queue/<name>/...` takes the queue route directly; the capture step is skipped
  for it.
...
which counts every publish a matching queue did not store — whether the append
failed, the queue's configuration could not be read, or the matching queues could
not be resolved at all. A non-zero and rising value means a queue is dropping the
traffic its pattern binds.
...
Consumers address queues through `$queue/`, so a captured message is rewritten
into that space before delivery. The source topic is preserved after the queue
root:

| Queue (pattern) | Published topic | Consumer receives |
| --- | --- | --- |
| `messages` (`m/#`) | `m/acme/temp` | `$queue/messages/m/acme/temp` |
| `messages` (`m/#`) | `$queue/messages/x` | `$queue/messages/x` (already canonical) |
| `m` (`m/#`) | `m/acme/temp` | `$queue/m/acme/temp` |

<Callout type="warn">
  The last row is a rough edge. When the published topic already begins with the
  queue's own name, that leading level is absorbed rather than repeated, so
  `$queue/m/acme/temp` is ambiguous between a capture of `m/acme/temp` in queue
  `m` and an explicit publish to `$queue/m/acme/temp`. Consumers that need the
  exact source topic should not reconstruct it from the delivery address. Naming
  a queue differently from the first level of its pattern avoids the case
  entirely.
</Callout>
...
A consumer can subscribe to a subset of messages in a stream queue by using a
topic filter. The broker routes only matching messages to the consumer while
...
See [Storage internals](/architecture/storage) for on-disk format and retention behavior details.
...
See [Queue Replication Groups (Raft)](/architecture/raft-groups) for the model and tradeoffs.

See [Cluster configuration](/configuration/clustering) and [Clustering internals](/architecture/clustering) for the full picture.
