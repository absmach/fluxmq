...
sequenceDiagram
  ...
  participant A as Node A (any node)
  participant L as Queue leader
  participant B as Node B (consumer connected)
  P->>A: Publish to $queue/...
  A->>L: Forward publish (if not leader)
  L->>L: Append via Raft (durable)
  L->>L: Claim from log + group state
  L->>B: Route delivery (if consumer is on B)
  B-->>B: Deliver to live connection"
/>
...
For multi-queue workloads, use replication groups to shard hot queues and reduce contention.
See [Queue Replication Groups (Raft)](/architecture/raft-groups).

## Configuration Entry Points

- Cluster basics: [Cluster configuration](/configuration/clustering)
- Replication and tuning: [Configuration reference](/reference/configuration-reference)
