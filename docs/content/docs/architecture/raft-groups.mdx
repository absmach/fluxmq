...
> **Not available in this release.** Queue Raft replication is not shipped in
> FluxMQ 1.0. The configuration keys below still parse, so a file prepared for a
> later release loads, but setting `cluster.raft.enabled: true` is refused at
> startup. This page describes the intended shape of the feature and the
> configuration reserved for it.
