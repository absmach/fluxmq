...
| `$share/<group>/<filter>` | Shared subscriptions (MQTT) | Each message goes to one member of the share group, cluster-wide. |
...
Topic matching is trie-based. Retained messages are stored and replayed on subscribe. Shared subscriptions (`$share/...`) are matched under their bare topic filter and resolved to a single group member before delivery. The group spans the cluster: the publishing node takes its turn across every member, wherever that member is connected, and addresses the message to the one it chose. See [Shared Subscriptions](/concepts/shared-subscriptions).
...
- **Blocking hooks**: synchronous allow/deny and mutation callouts via `broker/hook/`
...
- `hooks.*` for blocking hook callouts
- `ratelimit.*` for rate limiting
- `webhook.*` for webhook delivery

See [Configuration reference](/reference/configuration-reference) for full details.
