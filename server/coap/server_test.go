...
func TestBuildPublishMessage(t *testing.T) {
	t.Run("with external id and content type", func(t *testing.T) {
		...
		assert.Equal(t, byte(0), msg.BrokerMeta.Delivery.QoS)
		assert.False(t, msg.BrokerMeta.Delivery.Retain)
		assert.Equal(t, "coap:1.2.3.4:5683", msg.BrokerMeta.Source.ClientID)
		assert.Equal(t, "application/senml+json", msg.PublisherMeta.ContentType)
		assert.Equal(t, coremessage.ProtocolCoAP, msg.BrokerMeta.Source.Protocol)
		assert.Equal(t, "ext-7", msg.BrokerMeta.Source.ExternalID)
	})
