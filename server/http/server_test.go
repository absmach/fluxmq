...
func TestBuildPublishMessage(t *testing.T) {
	t.Run("with external id and content type", func(t *testing.T) {
		...
		assert.Equal(t, byte(1), msg.BrokerMeta.Delivery.QoS)
		assert.True(t, msg.BrokerMeta.Delivery.Retain)
		assert.Equal(t, "http:1.2.3.4:1234", msg.BrokerMeta.Source.ClientID)
		assert.Equal(t, "application/senml+json", msg.PublisherMeta.ContentType)
		assert.Equal(t, message.ProtocolHTTP, msg.BrokerMeta.Source.Protocol)
		assert.Equal(t, "ext-42", msg.BrokerMeta.Source.ExternalID)
	})
	...
	t.Run("omits external id when empty", func(t *testing.T) {
		...
		require.NotNil(t, msg)
		assert.Equal(t, message.ProtocolHTTP, msg.BrokerMeta.Source.Protocol)
		assert.Empty(t, msg.BrokerMeta.Source.ExternalID)
		assert.Equal(t, "", msg.PublisherMeta.ContentType)
	})
}
